...
class Credit_Card_WC_We_Payments_Gateway extends WC_We_Payments_Gateway{
	...
	public function __construct() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'credit_card_js' ) ); // Enqueue JavaScript
		add_action( 'wp_enqueue_scripts', array( $this, 'payment_scripts' ) ); // Enqueue JavaScript
		add_action( 'wp_enqueue_scripts', array( $this, 'credit_card_css' ) ); // Enqueue CSS
	...
	public function webhook() {
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			// Decodifica os dados do webhook, assumindo que eles estão em JSON
    ...
    */
    ...
    public function process_payment( $order_id ) {
		...
		foreach ($order->get_items() as $item_id => $item) {
			...
			$order_item = wc_get_order($order->get_id())->get_item($item_id);
        ...
        $request_data = array(
            ...
            'callbackUrl' => site_url('/wc-api/woo_we_payments_credit_card'),
            'title' => array(
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => $this->url . '/v1/payin/payments/credit-card',
	...
	public function payment_fields() {
		...
		?>
			...
			<div class="woo-we-payments-card-container">
				...
				<input type="hidden" id="card_token" name="card_token" value="0">
			</div>
		<?php
			...
			document.addEventListener('DOMContentLoaded', async function() {
				if (typeof WEPayin !== 'undefined') {
					...
					async function generateCardToken(cardData) {
						try {
							const cardToken = await sdk.tokenize(cardData);
							return cardToken;
						} catch (error) {
					...
					document.addEventListener('change', function(event) {
						...
						const installmentsSelect = document.querySelector('select#woo-we-payments-installment')
						...
						// Check if the event target matches any of the specified input fields
						
						if (event.target === cvvInput || event.target === nameInput ||
							event.target === expDateInput || event.target === numberInput) {
							console.log('Campo de cartão foi alterado.');


							var expDateValue = expDateInput.value;
							var numberInputValue = numberInput.value.replace(/\s/g, '');
							var cvvInputValue = cvvInput.value;
							var nameInputValue = nameInput.value;
							var cpf = cpfInput.value.replace(/[.-]/g, '');
							var cardToken = document.getElementById('card_token');
				...
				const generatecard = document.getElementById('generatecard');
					if(event.target === name){
						...
						if (name.value.length == 0) {
							...
							document.getElementById('svgname').innerHTML = event.target.value;
						}
					}
					document.querySelector('.creditcard').classList.remove('flipped');

					if(event.target === cardnumber){
						...
						if (cardnumber.value.length == 0) {
							...
							document.getElementById('svgnumber').innerHTML = event.target.value;
						}
					}


					if (event.target === expirationdate) {
						document.querySelector('.creditcard').classList.remove('flipped');

						function expiryMask(event) {
							...
							if (event.target.value.length == 0) {
								...
								document.getElementById('svgexpire').innerHTML = event.target.value;
							}
						}
					...
					if(event.target === securitycode){
						...
						if (securitycode.value.length == 0) {
							document.getElementById('svgsecurity').innerHTML = '123';
						} else {
							document.getElementById('svgsecurity').innerHTML = event.target.value;
						}
					}
				});
